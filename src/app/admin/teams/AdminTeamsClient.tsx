...
export default function AdminTeamsClient({ initialTeams }: { initialTeams: AdminTeam[] }) {
  ...
  const [editingGroup, setEditingGroup] = useState<{ [id: string]: string }>({});
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (logoFile) logoFileId = await uploadImage(logoFile);
  ...
  async function onUpdateGroup(teamId: string, newGroup: string) {
    setLoading(true);
    try {
      ...
      setEditingGroup((prev) => { const n = { ...prev }; delete n[teamId]; return n; });
      await refresh();
    } catch (err: any) {
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm text-white/60">Add squads, upload logos, assign groups, and manage members.</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {teams.length === 0 ? (
          ...
          teams.map((t) => {
            const pendingGroup = editingGroup[t._id] ?? t.group;
            const isDirty = editingGroup[t._id] !== undefined && editingGroup[t._id] !== t.group;
            return (
              <div key={t._id} className="card p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-white/50">{t.memberCount} members</div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-white/50">Group</span>
                      <select
                        className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-sm font-black text-emerald-300 outline-none focus:border-emerald-500/40"
                        value={pendingGroup}
                        onChange={(e) => setEditingGroup((prev) => ({ ...prev, [t._id]: e.target.value }))}
                        disabled={loading}
                      >
                        {GROUP_OPTIONS.map((g) => (
                          <option key={g} value={g}>Group {g}</option>
                      ...
                      {isDirty && (
                        <button
                          onClick={() => onUpdateGroup(t._id, pendingGroup)}
                          disabled={loading}
                          className="rounded-lg border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-xs font-black text-emerald-200 transition hover:bg-emerald-500/20 disabled:opacity-60"
                        >
                          Save
                        </button>
                      )}
