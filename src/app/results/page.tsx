...
export default async function ResultsPage() {
  try {
    ...
    const groupStageFixtures = allFixtures.filter((f: any) => !f.stage || f.stage === "group");

    // Get unique group names
    const groupNamesSet = new Set<string>();
    teams.forEach((t: any) => {
      groupNamesSet.add(t.group || "A");
    });
    const groupNames = Array.from(groupNamesSet).sort();

    // Calculate standings for each group
    const groupStandingsData = groupNames.map((group) => {
      const groupTeams = teams.filter((t: any) => (t.group || "A") === group);
      const standings = computeStandings({
        fixtures: groupStageFixtures,
        teams: groupTeams.map((t: any) => ({
          _id: t._id,
          name: t.name,
          logoFileId: t.logoFileId || null,
        })),
        pointsRules,
      });
    ...
    // Get completed matches for recap
      ...
      <section className="space-y-5 sm:space-y-7">
        ...
        <div className="grid gap-4 lg:gap-5 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3 space-y-5">
            {groupStandingsData.length === 0 ? (
              <div className="card p-6 text-sm text-white/60">No teams found.</div>
            ) : (
              groupStandingsData.map(({ group, standings }) => (
                    ...
                    <table className="w-full min-w-[480px] border-separate border-spacing-y-2 text-xs sm:text-sm">
                      ...
                      <tbody>
                        {standings && standings.length > 0 ? (
                              ...
                              <td className="px-1">
                                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                                  {r.logoFileId ? (
                                    ...
                                    <span className="flex h-6 w-6 sm:h-8 sm:w-8 flex-shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-[9px] font-black text-white/60">
                                      T
                                    </span>
                                  )}
                          ...
                          <tr>
                            <td colSpan={8} className="text-center py-4 text-xs text-white/40">
                              No standings yet
                            </td>
                          </tr>
                        )}
                  ...
                  return (
                    <div key={String(f._id)} className="rounded-xl border border-white/10 bg-white/5 p-2 sm:p-3">
                      <div className="flex items-center justify-between gap-2 sm:gap-3">
                        ...
                        <div className="text-right text-base sm:text-lg font-black text-emerald-300 flex-shrink-0">
