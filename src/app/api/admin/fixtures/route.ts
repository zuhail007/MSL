...
function mapFixture(f: any, teamsById: Map<string, any>) {
...
export async function GET() {
  ...
  return NextResponse.json(fixtures.map((f) => mapFixture(f, teamsById)));
}
...
export async function POST(req: Request) {
  ...
  const fixture = await FixtureModel.create({
    ...
    status,
    homeScore:
      status === "completed" ? (parsed.data.homeScore ?? null) : null,
    awayScore:
      status === "completed" ? (parsed.data.awayScore ?? null) : null,
  });
